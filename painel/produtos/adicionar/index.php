...
<?php
  ...
  if( $formdata ) {
    ...
    $debug_log = "DEBUG DESCRIÇÃO - " . date('Y-m-d H:i:s') . "\n";
    $debug_log .= "Raw POST: " . var_export($_POST['descricao'], true) . "\n";
    $debug_log .= "Hex: " . bin2hex($_POST['descricao']) . "\n";
    $debug_log .= "Contém \\n: " . (strpos($_POST['descricao'], "\n") !== false ? 'SIM' : 'NÃO') . "\n";
    $debug_log .= "Contém \\\\n: " . (strpos($_POST['descricao'], "\\n") !== false ? 'SIM' : 'NÃO') . "\n";
    $debug_log .= "---\n\n";
    file_put_contents('debug_descricao.log', $debug_log, FILE_APPEND);
    
    // Para new_produto que usa consulta direta, mantemos o escape
    ...
    for ( $x=0; $x < count( $variacao ); $x++ ){
      ...
      for( $y=0; $y < count( $variacao[$x]['item'] ); $y++ ){
        ...
        $variacao[$x]['item'][$y]['estoque_item'] = jsonsave( mysqli_real_escape_string( $db_con,$variacao[$x]['item'][$y]['estoque_item'] ) );
        $variacao[$x]['item'][$y]['quantidade_disponivel'] = jsonsave( mysqli_real_escape_string( $db_con,$variacao[$x]['item'][$y]['quantidade_disponivel'] ) );
      }
    }
    ...
    // Campos do link de afiliado
    $link_afiliado_ativo = mysqli_real_escape_string( $db_con, $_POST['link_afiliado_ativo'] );
    $link_afiliado_url = mysqli_real_escape_string( $db_con, $_POST['link_afiliado_url'] );
    ...
    if( !$checkerrors ) {
        ...
        // Salvar link de afiliado se o produto foi criado com sucesso
        if($pid) {
          save_link_afiliado($pid, $eid, $link_afiliado_ativo, $link_afiliado_url);
        }
...
<div class="middle minfit bg-gray">
	...
	<div class="container">
      ...
      <form id="the_form" class="form-default" method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="form-field-default">
                  ...
                  <textarea rows="6" name="descricao" placeholder="Descrição do seu produto"><?php echo htmlspecialchars( $_POST['descricao'] ); ?></textarea>
          ...
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="form-field-default">
                  ...
                  <label>Link de Afiliado/Vendas Externa:</label>
                  <span class="form-tip">Ative para redirecionar as compras para um link externo de vendas ou afiliado.</span>
                  <div class="form-field-radio">
                    <input type="radio" name="link_afiliado_ativo" value="1" element-show=".elemento-link-afiliado" <?php if( $_POST['link_afiliado_ativo'] == 1 ){ echo 'CHECKED'; }; ?>> Ativar Link Externo
                  </div>
                  <div class="form-field-radio">
                    <input type="radio" name="link_afiliado_ativo" value="0" element-hide=".elemento-link-afiliado" <?php if( $_POST['link_afiliado_ativo'] != 1 ){ echo 'CHECKED'; }; ?>> Usar Sistema Normal
                  </div>
                  <div class="clear"></div>
          ...
          <div class="row elemento-link-afiliado <?php if( $_POST['link_afiliado_ativo'] != '1' ){ echo 'elemento-oculto'; } ?>">

            <div class="col-md-12">

              <div class="form-field-default">

                  <label>URL do Link de Vendas:</label>
                  <span class="form-tip">Insira o link completo para onde os clientes serão redirecionados ao clicar em "Comprar".</span>
                  <input type="url" name="link_afiliado_url" placeholder="https://exemplo.com/produto" value="<?php echo htmlclean( $_POST['link_afiliado_url'] ); ?>">
...
$(document).ready(function() {
  $("#btn_import_xml").on("click", function() {
    var xmlUrl = $("#import_xml_url").val().trim();
    if (xmlUrl === "") {
      alert("Por favor, insira a URL do feed XML.");
      $("#import_xml_url").focus();
      return;
    }

    var button = $(this);
    var statusDiv = $("#import_xml_status");
    var originalButtonText = button.html();

    button.html('Importando... <i class="lni lni-spinner-arrow"></i>').prop('disabled', true);
    statusDiv.html('<div class="alert alert-info">Processando o XML... Por favor, aguarde.</div>');

    $.ajax({
      url: "<?php echo just_url(); ?>/_core/_ajax/importar_produtos_xml.php",
      type: "POST",
      data: { xml_url: xmlUrl },
      dataType: "json",
      success: function(response) {
        ...
        if (response && response.success) {
          var message = '<div class="alert alert-success">Importação concluída!<br>';
          message += 'Produtos importados com sucesso: ' + response.imported_count + '<br>';
          message += 'Produtos já existentes/ignorados: ' + response.skipped_count + '<br>';
          if (response.errors && response.errors.length > 0) {
            message += 'Erros encontrados: <br><ul>';
          ...
          // Opcional: Recarregar a página ou limpar o formulário se necessário
          // alert("Importação concluída! Verifique os logs para mais detalhes.");
          // window.location.reload(); // Ou redirecionar para a lista de produtos
        } else {
          var errorMsg = '<div class="alert alert-danger">Erro na importação: ';
          errorMsg += (response.message || "Não foi possível processar o XML.") + '</div>';
          statusDiv.html(errorMsg);
        }
      },
      error: function(jqXHR, textStatus, errorThrown) {
        button.html(originalButtonText).prop('disabled', false);
        var errorMsgText = "Erro na comunicação com o servidor: ";
        if (jqXHR.status === 0) {
            errorMsgText += 'Não foi possível conectar. Verifique sua rede.';
        } else if (jqXHR.status == 404) {
            errorMsgText += 'Script de importação não encontrado (404).';
        } else if (jqXHR.status == 500) {
            errorMsgText += 'Erro interno no servidor (500). Verifique os logs do PHP.';
        } else if (textStatus === 'parsererror') {
            errorMsgText += 'Falha ao analisar a resposta do servidor (JSON esperado). Verifique a saída do script PHP.';
        } else if (textStatus === 'timeout') {
            errorMsgText += 'Tempo limite da requisição esgotado.';
        } else {
            errorMsgText += 'Erro desconhecido ('+ jqXHR.status + '): ' + errorThrown;
        }
        statusDiv.html('<div class="alert alert-danger">' + errorMsgText + '</div>');
      }
    });
  });
});
  ...
  function reordena() {
    ...
    $( ".variacao" ).each(function() {
      ...
      $( this ).find(".item").each(function() {
        ...
        $( this ).find(".item-estoque").attr("name","variacao["+variacao_pai+"][item]["+item+"][estoque_item]");
        $( this ).find(".item-quantidade-disponivel").attr("name","variacao["+variacao_pai+"][item]["+item+"][quantidade_disponivel]");
        item++;
...
<script>
// Script para mostrar/ocultar o campo de link de afiliado
$(document).ready(function() {
    $('input[name="link_afiliado_ativo"]').change(function() {
        if($(this).val() == '1') {
            $('.elemento-link-afiliado').removeClass('elemento-oculto');
        } else {
            $('.elemento-link-afiliado').addClass('elemento-oculto');
        }
    });
});
</script>
